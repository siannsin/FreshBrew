...
final class AppWorkflowTests: XCTestCase {
    ...
    func testPackageHomepageServiceRejectsInvalidSuppliedURLBeforeOpeningOrCaching() async throws {
        let defaults = InMemoryPreferencesStore()
        let store = PackageHomepageStore(defaults: defaults)
        let invalidURL = try XCTUnwrap(URL(string: "https://user:password@example.com/package"))
        var openedURL: URL?
        let service = PackageHomepageService(
            homepageResolver: StubPackageHomepageResolver(
                homepageURL: URL(string: "https://example.com/fallback")!
        ...
        do {
            _ = try await service.openPage(
                packageID: "formula:ripgrep",
                packageName: "ripgrep",
                kind: .formula,
                homepageURL: invalidURL
            )
            XCTFail("Expected the supplied homepage to be rejected")
        } catch let error as PackageHomepageError {
            XCTAssertEqual(error, .invalidURL(invalidURL.absoluteString))
        }

        XCTAssertNil(openedURL)
        XCTAssertNil(store.url(for: "formula:ripgrep"))
    }

    func testPackageHomepageServiceRejectsInvalidResolvedURLBeforeOpeningOrCaching() async throws {
        let defaults = InMemoryPreferencesStore()
        let store = PackageHomepageStore(defaults: defaults)
        let invalidURL = try XCTUnwrap(URL(string: "file:///tmp/package"))
        var openedURL: URL?
        let service = PackageHomepageService(
            homepageResolver: StubPackageHomepageResolver(homepageURL: invalidURL),
            store: store,
            openURL: { url in
                openedURL = url
                return true
        ...
        do {
            ...
            XCTFail("Expected the resolved homepage to be rejected")
        } catch let error as PackageHomepageError {
            XCTAssertEqual(error, .invalidURL(invalidURL.absoluteString))
        }
    ...
    func testPackageHomepageServiceIgnoresInvalidCachedURL() async throws {
        let defaults = InMemoryPreferencesStore()
        defaults.set(
            ["formula:ripgrep": "https://user@example.com/package"],
            forKey: "packageHomepageURLs"
        )
        let store = PackageHomepageStore(defaults: defaults)
        let fallbackURL = try XCTUnwrap(URL(string: "https://example.com/ripgrep"))
        var openedURL: URL?
        let service = PackageHomepageService(
            homepageResolver: StubPackageHomepageResolver(homepageURL: fallbackURL),
            store: store,
            openURL: { url in
                openedURL = url
                return true
        ...
        XCTAssertEqual(openedURL, fallbackURL)
        XCTAssertEqual(store.url(for: "formula:ripgrep"), fallbackURL)
    }
